...
public class OperatorDrive extends CommandBase {
    ...
    protected void execute() {
        ...
        if(Math.abs(moveAxis) >= 0.02 || Math.abs(rotateAxis) >= 0.02) {
            ...
            driveTrain.arcadeDrive((Math.abs(moveAxis) >= 0.05) ? moveAxis : 0, 
                    (Math.abs(rotateAxis) >= 0.05) ? rotateAxis : 0);
    ...
    protected void end() {
        SmartDashboard.putNumber("Move Axis", 0);
        SmartDashboard.putNumber("Rotate Axis", 0);
    }
